...
def plot_to_base64(plot_function, dpi=100, format='png', quality=90):
    ...
    try:
        ...
        if format.lower() in ('jpg', 'jpeg'):
            plt.savefig(buf, format=format, bbox_inches='tight', dpi=dpi, quality=quality)
        elif format.lower() == 'webp':
            plt.savefig(buf, format=format, bbox_inches='tight', dpi=dpi, quality=quality)
        else:
            # Default to PNG
            plt.savefig(buf, format='png', bbox_inches='tight', dpi=dpi, 
                        transparent=False, optimize=True, compress_level=9)
...
def generate_task_completion_chart(task_data):
    ...
    def plot_chart():
        ...
        colors = ['#f44336' if rate < 50 else 
                ...
                '#4caf50' for rate in completion_rates]
...
def calculate_interaction_metrics(tracking_data, duration_seconds=None):
    ...
    if isinstance(tracking_data, dict) and any(k in tracking_data for k in ['mouse_movements', 'mouse_clicks', 'keystrokes']):
        metrics = {
