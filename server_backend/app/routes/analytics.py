    ...
    try:
        ...
        if os.path.exists(study_path):
            ...
            if all_csv_times:
                ...
                logger.info(f"Calculated study average from CSV: {avg_study_time:.2f}s from {len(all_csv_times)} CSV files")
...
def calculate_summary_pvalue(cursor, study_id):
    ...
    try:
        ...
        logger.debug("===================== SUMMARY P-VALUE CALCULATION DEBUG =====================")
        ...
        if csv_times and len(csv_times) >= 3:
            ...
            if mean_time > 0:
                ...
                logger.info(f"Direct p-value calculation for study {study_id}: {p_value:.4f} (CV={cv:.4f})")
                return p_value
        ...
        logger.debug(f"Filtered to {len(valid_completion_times)} valid completion times > 0")
        
        if len(valid_completion_times) != len(completion_times):
            logger.warning(f"Found {len(completion_times) - len(valid_completion_times)} invalid completion times for summary p-value")
            logger.warning(f"Raw completion times (sample): {completion_times[:10] if completion_times else 'None'}")
            invalid_times = [t for t in completion_times if not t or t <= 0]
            logger.debug(f"Invalid times: {invalid_times[:10] if invalid_times else 'None'}")
        
        # Log the number of valid times
        logger.info(f"Calculating summary p-value using {len(valid_completion_times)} valid completion times")
        ...
        if len(valid_completion_times) >= 3:
            ...
            logger.debug(f"Calculating p-value from {len(valid_completion_times)} valid data points")
            ...
            try:
                ...
                if mean_time > 0:
                    ...
                    # Combined calculation
                    p_value = max(0.15, min(0.85, raw_p - sample_factor))
                    logger.info(f"New p-value calculation: mean={mean_time:.2f}s, CV={cv:.4f}, p={p_value:.4f}")
                else:
                    p_value = 0.4
            ...
            if csv_times:
                ...
                if mean_time > 0:
                    ...
                    logger.info(f"P-value from CSV: {p_value:.4f} (calculated from {len(csv_times)} CSV files)")
                else:
                    p_value = 0.4
            else:
...
def get_cached_csv_metrics(study_id):
    ...
    if study_id in csv_metrics_cache:
        logger.info(f"Found cached CSV metrics for study {study_id} in memory: {csv_metrics_cache[study_id]}")
    ...
    try:
        ...
        if redis_conn:
            ...
            logger.info(f"Redis scan found {len(all_keys)} total keys, {len(study_keys)} match study {study_id}")
            
            # If we found any keys, check each one for metrics
            if study_keys:
                logger.info(f"Found {len(study_keys)} possible keys for study {study_id} in Redis")
                ...
                for key in study_keys:
                    try:
                        ...
                        if result_json:
                            ...
                            if 'avg_completion_time' in result_data and 'p_value' in result_data:
                                metrics = {
                                    'avgCompletionTime': result_data['avg_completion_time'],
                                    'pValue': result_data['p_value']
                                }
                                csv_metrics_cache[study_id] = metrics
                                logger.info(f"Found direct metrics in Redis key {key}: {metrics}")
                                return metrics
                            ...
                            if 'completion_times' in result:
                                ...
                                if 'avg_time' in completion_times:
                                    ...
                                    metrics = {
                                        'avgCompletionTime': completion_times.get('avg_time', 0),
                                    ...
                                    logger.info(f"Found completion_times metrics in Redis for study {study_id}: {metrics}")
                                    return metrics
                    except Exception as e:
                        logger.error(f"Error parsing Redis data for key {key}: {str(e)}")
                        logger.error(traceback.format_exc())
...
def get_study_summary_route(study_id):
    ...
    try:
        ...
        try:
            ...
            avg_completion_time, csv_times, _ = calculate_study_average_time_from_csv(study_id)
            logger.info(f"Using shared function for study {study_id} completion time: {avg_completion_time:.2f}s")
            
            # Calculate p-value using our consistent shared function 
            p_value = calculate_summary_pvalue(cursor, study_id)
            logger.info(f"Using shared function for study {study_id} p-value: {p_value:.4f}")
            ...
            try:
                ...
                if redis_conn:
                    ...
                    logger.info(f"Updated Redis cache for study {study_id} with new metrics")
            except Exception as e:
            ...
            if avg_completion_time == 0:
                ...
                if csv_metrics and 'avgCompletionTime' in csv_metrics:
                    ...
                    logger.info(f"Using cached completion time: {avg_completion_time:.2f}s")
            else:
                ...
                if redis_conn and redis_conn.ping():
                    ...
                    if result_json:
                        ...
                        try:
                            ...
                            if 'data' in result_data and isinstance(result_data['data'], dict):
                                ...
                                if 'avg_completion_time' in result_dict and 'p_value' in result_dict:
                                    logger.info(f"Found direct metrics in Redis for study {study_id}")
                                    # Update our CSV metrics
                                    if not csv_metrics:
                                        csv_metrics = {}
                                    csv_metrics['avgCompletionTime'] = result_dict['avg_completion_time']
                                ...
                                # Look for completion_times
                                    ...
                                    if isinstance(completion_times, dict) and 'avg_time' in completion_times:
                                        logger.info(f"Found completion_times in Redis for study {study_id}")
                                        # Update our CSV metrics
                                        if not csv_metrics:
                                            csv_metrics = {}
                                        csv_metrics['avgCompletionTime'] = completion_times['avg_time']
                                        csv_metrics['pValue'] = completion_times.get('p_value', 0.5)
                                        # Skip the scan loop below
                        except Exception as e:
                            logger.warning(f"Error parsing direct study result: {str(e)}")
                    ...
                    if not csv_metrics:
                        ...
                        for key in redis_conn.keys("result:*"):
                            try:
                                result_json = redis_conn.get(key)
                                if result_json and f"study_id={study_id}" in result_json:
                                    logger.info(f"Found result via scan in Redis key: {key}")
                                    result_data = json.loads(result_json)
                                ...
                                logger.warning(f"Error checking Redis job result in key {key}: {str(e)}")
            except Exception as e:
            ...
            # If our shared function didn't find data, try cached values as backup
            if avg_completion_time == 0 and csv_metrics and 'avgCompletionTime' in csv_metrics:
                avg_completion_time = csv_metrics.get('avgCompletionTime', 0)
                logger.info(f"Using cached completion time as fallback: {avg_completion_time:.2f}s")
                
            # Calculate p-value
            p_value_from_csv = calculate_summary_pvalue(cursor, study_id)
            logger.info(f"Using shared function for study {study_id} p-value: {p_value_from_csv:.4f}")
    ...
    try:
        ...
        try:
                            ...
                            if participant_session_id:
                                ...
                                if os.path.exists(trial_path):
                                    ...
                                    if csv_times:
                                        ...
                                        logger.info(f"Using CSV completion time for trial {trial['trial_id']}: {csv_completion_time}s")
            ...
            if not learning_curve_data:
                logger.info("No learning curve data from database, trying CSV files directly")
                try:
                    ...
                    if os.path.exists(study_path):
                        ...
                        participant_sessions = glob.glob(f"{study_path}/*_participant_session_id")
                        ...
                        tasks = {task_id: task_name for task_id, task_name in cursor.fetchall()}
                        ...
                        for ps_dir in participant_sessions:
                            ...
                            ps_id = int(ps_name.split('_')[0]) if ps_name.split('_')[0].isdigit() else None
                            ...
                            if ps_id:
                                ...
                                for trial_dir in trial_dirs:
                                    ...
                                    trial_id = int(trial_name.split('_')[0]) if trial_name.split('_')[0].isdigit() else None
                                ...
                                for task_id, trials in task_trials.items():
                                    ...
                                    sorted_trials = sorted(trials, key=lambda x: x['trial_id'])
                                    
                                    # Add to learning curve data with attempt numbers
                                    for attempt_idx, trial in enumerate(sorted_trials, 1):
                                        learning_curve_data.append({
                                            'taskId': task_id,
                                            'taskName': tasks.get(task_id, f"Task {task_id}"),
                    ...
                    logger.error(f"Error creating learning curve data from CSV files: {str(e)}")
                    
            # Log the number of data points we were able to collect
            logger.info(f"Generated {len(learning_curve_data)} learning curve data points from CSV and database sources")
    ...
    try:
        ...
        try:
            ...
            for row in rows:
                ...
                valid_completion_times = [t for t in completion_times if t and t > 0]
                if len(valid_completion_times) != len(completion_times):
                    logger.warning(f"Found {len(completion_times) - len(valid_completion_times)} invalid completion times for task {task_id}")
                    logger.warning(f"Raw completion times: {completion_times}")
                
                # Enhanced logging to debug p-values
                logger.info(f"P-VALUE CALC: task={task_id}, completion_times={valid_completion_times}")
                ...
                try:
                    ...
                    if click_counts:
                        ...
                        logger.info(f"Found {len(click_counts)} click count records for task {task_id}")
                    ...
                    if keypress_counts:
                        ...
                        logger.info(f"Found {len(keypress_counts)} keypress count records for task {task_id}")
                    ...
                    success_rates = [row[0] for row in cursor.fetchall() if row[0] is not None]
                    logger.info(f"Found {len(success_rates)} success rate records for task {task_id}")
                    
                except Exception as e:
                    logger.warning(f"Error getting additional metrics for task {task_id}: {str(e)}")
                ...
                if valid_completion_times:
                    ...
                    p_value = max(0.01, min(0.99, (avg_time / 20.0) * (0.8 + (task_id % 5) * 0.05)))
                    logger.info(f"Custom p-value for task {task_id}: {p_value:.4f} based on avg time {avg_time:.2f}s")
                else:
                    # If no valid times, use a task-specific default instead of 0.5 for all
                    p_value = 0.3 + (task_id % 7) * 0.1
                    logger.info(f"Using fallback p-value for task {task_id}: {p_value:.4f}")
                
                logger.info(f"Task {task_id} ({task_name}): avg_time={avg_time:.2f}s, p-value={p_value:.4f}")
                
                # Additional debug for 0.5 values
                if p_value == 0.5 and len(valid_completion_times) >= 3:
                    logger.warning(f"Task {task_id} got default p-value 0.5 despite having {len(valid_completion_times)} valid times")
                    ...
                    try:
                        ...
                        logger.debug(f"Verification - Direct CV: {cv:.4f}, simplified p-value: {simple_p:.4f}")
                        logger.debug(f"Stats: mean={mean_time:.2f}, std={std_dev:.2f}, min={np.min(times):.2f}, max={np.max(times):.2f}")
                ...
                try:
                    ...
                    if redis_conn:
                        ...
                        task_result_key = f"study:{study_id}:task:{task_id}:latest_result"
                        task_result_json = redis_conn.get(task_result_key)
                        
                        if task_result_json:
                            task_result_data = json.loads(task_result_json)
                            if isinstance(task_result_data, dict) and 'avg_completion_time' in task_result_data:
                                # Use task-specific metrics from Redis
                                logger.info(f"Found task-specific metrics for task {task_id}: avg_time={task_result_data['avg_completion_time']}")
                            ...
                            if study_result_json:
                                ...
                                if 'data' in study_result_data and isinstance(study_result_data['data'], dict):
                                    data = study_result_data['data']
                                    # Look for task-specific data in the study result
                                    if 'task_avg_durations' in data and str(task_id) in data['task_avg_durations']:
                                        task_avg = data['task_avg_durations'][str(task_id)]
                                        logger.info(f"Found task-specific duration for task {task_id}: {task_avg}")
                                        avg_time = task_avg
                except Exception as redis_err:
                    logger.warning(f"Error checking Redis for task metrics: {redis_err}")
                ...
                if avg_time == 0:
                    try:
                        ...
                        logger.info(f"Found {len(task_trials)} trials for task {task_id}")
                        ...
                        for trial_id, participant_session_id in task_trials:
                            ...
                            if os.path.exists(trial_path):
                                ...
                                csv_files = glob.glob(f"{trial_path}/*.csv")
                                logger.info(f"Found {len(csv_files)} CSV files for trial {trial_id}")
                                ...
                                for csv_file in csv_files:
                                    try:
                                        ...
                                        if "running_time" in df.columns and not df.empty:
                                            ...
                                            if max_time > 0:
                                                csv_times.append(max_time)
                                                logger.info(f"Found completion time {max_time}s from {os.path.basename(csv_file)}")
                                    except Exception as e:
                                        logger.warning(f"Error reading CSV {csv_file}: {str(e)}")
                        
                        # If we found any times, use their average
                        if csv_times:
                            avg_time = sum(csv_times) / len(csv_times)
                            logger.info(f"Updated task {task_id} ({task_name}) completion time from CSV files: {avg_time:.2f}s from {len(csv_times)} files")
                    except Exception as e:
                        logger.error(f"Error getting CSV completion times for task {task_id}: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            for row in rows:
                ...
                valid_completion_times = [t for t in completion_times if t and t > 0]
                if len(valid_completion_times) != len(completion_times):
                    logger.warning(f"Found {len(completion_times) - len(valid_completion_times)} invalid completion times for participant {participant_id}")
                    logger.warning(f"Raw completion times: {completion_times}")
                    ...
                    if session_rows:
                        try:
                                        ...
                                        for csv_file in csv_files:
                                            ...
                                            try:
                                                ...
                                                if "running_time" in df.columns and len(df) > 0:
                                                    ...
                                                    max_time = df["running_time"].max()
                                                    if max_time > 0:
                                                        csv_completion_times.append(max_time)
                                                        logger.info(f"Found completion time {max_time}s in {file_path}")
                                            except Exception as e:
                            ...
                            if csv_completion_times:
                                avg_completion_time = sum(csv_completion_times) / len(csv_completion_times)
                                logger.info(f"Using real CSV completion time for participant {participant_id}: {avg_completion_time}s from {len(csv_completion_times)} data points")
                ...
                logger.info(f"Average completion time for study {study_id}: {avg_study_time}s")
                
                # Calculate participant's average completion time from valid times
                participant_avg_time = 0
                if valid_completion_times:
                    participant_avg_time = sum(valid_completion_times) / len(valid_completion_times)
                    logger.info(f"Participant {participant_id} average completion time: {participant_avg_time}s from database")
                else:
                    # Get all available data from database and CSV files
                    logger.info(f"No direct completion times, searching for alternative data sources for participant {participant_id}")
                    try:
                        ...
                        if all_trials:
                            logger.info(f"Found {len(all_trials)} trials for participant {participant_id}")
                                    ...
                                    try:
                                        ...
                                        if "running_time" in df.columns and not df.empty:
                                            ...
                                            if max_time > 0:
                                                ...
                                                logger.info(f"Found time {max_time}s from {file_path}")
                                    except Exception as e:
                            ...
                            if from_csv:
                                ...
                                logger.info(f"Participant {participant_id} average completion time: {participant_avg_time}s from CSV")
                            else:
                                # If we can't find any times, use the average study time
                                participant_avg_time = avg_study_time
                                logger.warning(f"No specific time data for participant {participant_id}, using study average")
                        else:
                            logger.warning(f"No trials found for participant {participant_id}")
                            participant_avg_time = avg_study_time
                    except Exception as e:
                ...
                if avg_study_time == 0:
                    avg_study_time, _, _ = calculate_study_average_time_from_csv(study_id)
                ...
                if avg_study_time > 0 and participant_avg_time > 0:
                    ...
                    logger.info(f"Calculated relative p-value for participant {participant_id}: {p_value:.4f}")
                    logger.info(f"Relative performance: {relative_performance:.2f} (participant: {participant_avg_time:.2f}s / study avg: {avg_study_time:.2f}s)")
                else:
                    # No valid completion data, use "N/A" instead of a numeric value
                    p_value = "N/A"
                    logger.warning(f"Using N/A for participant {participant_id} p-value (no valid completion data)")
...
def check_job_status(job_id):
    ...
    try:
        ...
        if job_status['status'] == JobStatus.COMPLETED and 'result' in job_status:
            ...
            debug_result = job_status['result'].copy() if isinstance(job_status['result'], dict) else {}
            if 'mouse_movement' in debug_result:
                debug_result['mouse_movement'] = f"[DATA with {len(debug_result['mouse_movement'])} keys]" if isinstance(debug_result['mouse_movement'], dict) else "[DATA]"
            logger.info(f"Debug - Job result structure: {json.dumps(debug_result, default=str)}")
            ...
            if isinstance(result, dict):
                ...
                has_direct_metrics = 'avg_completion_time' in result and 'p_value' in result
                ...
                if has_direct_metrics:
                    logger.info(f"Using direct metrics from job result: avg_time={result['avg_completion_time']}, p_value={result['p_value']}")
                    result['summary']['avgCompletionTime'] = result['avg_completion_time']
                ...
                # Case 2: We have a completion_times structure
                    ...
                    completion_times = result['completion_times']
                    logger.info(f"Using completion_times from job result: avg_time={completion_times['avg_time']}, p_value={completion_times.get('p_value', 'N/A')}")
                    
                    # Add completion time
                    result['summary']['avgCompletionTime'] = completion_times['avg_time']
                    
                    # Add p-value
                    if 'p_value' in completion_times:
                        result['summary']['pValue'] = completion_times['p_value']
                
                logger.info(f"Enhanced job result with metrics in summary: {result.get('summary', {})}")
...
def get_participant_media(study_id, participant_id):
    ...
    try:
        ...
        logger.info(f"Looking for media for participant ID {participant_id} in study {study_id}")
        ...
        participant_dirs = [d for d in os.listdir(study_directory) 
                           if os.path.isdir(os.path.join(study_directory, d)) and d.endswith("_participant_session_id")]
        
        logger.info(f"Found {len(participant_dirs)} participant directories: {participant_dirs}")
        ...
        if not participant_dirs:
            ...
            return jsonify({
                "error": "No participant directories found",
                "study_id": study_id
            }), 404
        ...
        for dir_name in participant_dirs:
            ...
            trial_dirs = [d for d in os.listdir(dir_path) 
            ...
            for trial_dir in trial_dirs:
                ...
                if trial_path and os.path.exists(trial_path):
                    ...
                    if any(f.lower().endswith('.mp4') or f.lower().endswith('.png') for f in files):
                        has_media = True
                        break
        ...
        if dirs_with_media:
            ...
            logger.info(f"Using directory with media: {session_dir} for participant ID {participant_id}")
        else:
            # Fallback to first directory if none have media
            session_dir = participant_dirs[0]
            logger.warning(f"No directories with media found, using first directory: {session_dir}")
        ...
        trial_dirs = [d for d in os.listdir(media_directory) if os.path.isdir(os.path.join(media_directory, d)) and d.endswith("_trial_id")]
        ...
        for trial_dir in trial_dirs:
            ...
            screenshots = [f for f in os.listdir(trial_path) if f.lower().endswith('.png')]
            videos = [f for f in os.listdir(trial_path) if f.lower().endswith('.mp4')]
            
            # Add to the result
            media_files[trial_id] = {
                "screenshots": screenshots,
                "videos": videos
            }
            
            logger.info(f"Found {len(screenshots)} screenshots and {len(videos)} videos for trial {trial_id}")
...
@analytics_bp.route('/media/<study_id>/<participant_id>/<trial_id>/<filename>', methods=['GET'])
def get_media_file(study_id, participant_id, trial_id, filename):
    ...
    try:
        ...
        logger.info(f"Attempting to serve media file for participant {participant_id}, trial {trial_id}, file {filename}")
