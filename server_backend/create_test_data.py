...
def create_test_study(connection, owner_user_id, study_name=None):
    ...
    if not study_name:
        study_name = f"Test Study - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    ...
    try:
        ...
        cursor.execute("SELECT study_user_role_type_id FROM study_user_role_type WHERE study_user_role_description = 'Owner'")
        result = cursor.fetchone()
        if not result:
            # Try to get any role if "Owner" doesn't exist
            cursor.execute("SELECT study_user_role_type_id FROM study_user_role_type LIMIT 1")
            result = cursor.fetchone()
...
def create_test_tasks(connection, study_id, num_tasks=2):
    ...
    task_names = [
        "Memory Recall", "Pattern Recognition", 
        "Reaction Time", "Attention Test", 
        "Cognitive Load", "Visual Search"
    ]
...
def create_test_measurement_options(connection, num_options=3):
    ...
    option_names = [
        "Mouse Movement", "Mouse Clicks", "Keyboard Input", 
        "Eye Tracking", "Screen Recording"
    ]
    
    try:
        # Check which options already exist
        cursor.execute("SELECT measurement_option_id, measurement_option_name FROM measurement_option")
...
def create_test_participants(connection, study_id, num_participants=5):
    ...
    try:
        ...
        # Get education types
        cursor.execute("SELECT highest_education_type_id FROM highest_education_type LIMIT 3")
        ...
        connection.commit()
        print(f"Created {len(participant_session_ids)} participants with sessions for study {study_id}")
...
def create_test_trials(connection, participant_session_ids, task_ids, factor_ids, measurement_option_ids):
    ...
    try:
        for session_id in participant_session_ids:
            ...
            cursor.execute("SELECT created_at, ended_at, study_id FROM participant_session WHERE participant_session_id = %s", (session_id,))
            ...
            session_dir = os.path.join(data_dir, f"{study_id}_study_id", f"{session_id}_participant_session_id")
            ...
            for task_id in task_ids:
                for factor_id in factor_ids:
                    ...
                    trial_start = session_start + timedelta(seconds=random.uniform(0, session_length * 0.8))
                    completed = random.random() > 0.2  # 80% completion rate
                    
                    if completed:
                        trial_end = trial_start + timedelta(seconds=random.uniform(30, time_per_trial))
                        trial_duration = (trial_end - trial_start).total_seconds()
                    else:
                        trial_end = None
                        trial_duration = random.uniform(30, 120)  # Assume some activity before abandonment
                    ...
                    for option_id in measurement_option_ids:
                        option_name = measurement_options.get(option_id, f"Option_{option_id}")
...
def create_complete_test_dataset(user_id, study_name=None, num_participants=5, num_tasks=2, num_factors=2):
    ...
    participant_session_ids = create_test_participants(connection, study_id, num_participants)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create test data for analytics testing")
    parser.add_argument("user_id", type=int, help="User ID to own the test study")
    parser.add_argument("--name", "-n", type=str, help="Study name (defaults to auto-generated)")
    parser.add_argument("--participants", "-p", type=int, default=5, help="Number of participants to create")
    parser.add_argument("--tasks", "-t", type=int, default=2, help="Number of tasks to create")
    parser.add_argument("--factors", "-f", type=int, default=2, help="Number of factors to create")
