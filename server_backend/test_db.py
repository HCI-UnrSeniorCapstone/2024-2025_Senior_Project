...
def test_db_connection():
    try:
        ...
        cursor.execute("SELECT participant_session_id, participant_id, study_id FROM participant_session LIMIT 5")
        print("  Column headers: participant_session_id, participant_id, study_id")
        for (session_id, participant_id, study_id) in cursor:
            print(f"  Session ID: {session_id}, Participant ID: {participant_id}, Study ID: {study_id}")
