...
from tracking.utility.measure import pause_event, stop_event, data_storage_complete_event
...
class GlobalToolbar(QWidget):
    ...
    def setup_ui(self):
        ...
        # Start button
        start_layout, self.start_btn= create_btn("./icons/start.svg", "Start", self.start_session)
        layout.addLayout(start_layout)
        
        # Pause/Resume button
        pause_layout, self.pause_btn= create_btn("./icons/pause.svg", "Pause/Resume", self.pause_session)
        layout.addLayout(pause_layout)
        self.pause_btn.setEnabled(False) # starts disabled

        # Next button
        next_layout, self.next_btn = create_btn("./icons/next.svg", "Next Task", self.move_next_task)
        layout.addLayout(next_layout)
        self.next_btn.setEnabled(False) # starts disabled
                
        # Timer countdown
        self.timer_label = QLabel("No Time Limit")
        self.timer_label.setStyleSheet("color: white; font-size: 16px; padding: 5px 10px; border-radius: 12px; border: 1px solid #444")
        layout.addWidget(self.timer_label)

        # Participant's progress 
        self.progress = QLabel("           ")
        self.progress.setStyleSheet("color: white; font-size: 16px; padding: 5px 10px; border-radius: 12px; border: 1px solid #444")
        layout.addWidget(self.progress)

        # Help button
        help_layout, self.help_btn= create_btn("./icons/help.svg", "Help", self.open_help_menu)
        layout.addLayout(help_layout)
                       
        # Quit button
        quit_layout, self.quit_btn= create_btn("./icons/quit.svg", "Quit", self.leave_session)
    ...
    def load_session(self):
        ...
        with open('../frontend/public/sample_study.json', 'r') as file:
            data = json.load(file)
        self.session_id, self.tasks, self.factors, self.trials = self.parse_study_details(data)
    ...
    def move_next_task(self):
        ...
        trial_msg.setStandardButtons(QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel)
        ...
        if trial_msg.exec() == QMessageBox.StandardButton.Ok:
            ...
            trial_thread = threading.Thread(target=conduct_trial, args=(self.session_id, task, factor))
            trial_thread.start()       
