...
def test_screen_recording():
    output_loc = os.path.expanduser("~") # For testing purposes we will be saving to user's home dir
    filename = "sample"
    ...
    recording_thread = threading.Thread(target = record_screen, args = (output_loc, filename))
    ...
    assert os.path.exists(file_path) == True, f"Sample screen recording failed to generate at {file_path}"
