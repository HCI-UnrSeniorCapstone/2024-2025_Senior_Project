<template>
  <div class="bubble-chart-container">
    ...
    <div v-if="loading || asyncLoading" class="status-container">
      ...
      />
      <p class="mt-2">{{ asyncLoading ? 'Processing interaction data...' : 'Loading data...' }}</p>
      <p v-if="asyncLoading" class="text-caption">This may take a moment for large datasets</p>
    </div>
    
    <!-- Error state -->
    <div v-else-if="error || asyncError" class="status-container">
      <v-alert type="error" dense>{{ error || asyncError }}</v-alert>
      <v-btn v-if="asyncError" small color="primary" class="mt-3" @click="fetchZipData">
        Try Again
      </v-btn>
    </div>
...
// Computed properties
const hasData = computed(() => props.participantData && props.participantData.length > 0);
...
async function fetchZipData() {
  ...
  const studyIdNumeric = typeof props.studyId === 'string' ? parseInt(props.studyId, 10) : props.studyId;
  ...
  try {
    ...
    if (data && data.status === 'processing' && data.job_id) {
      ...
      asyncError.value = "Request timed out. The job may still be processing in the background.";
      asyncLoading.value = false;
...
function getChartColors() {
  ...
  if (xAxisMetric.value === 'mouseMovement' || yAxisMetric.value === 'mouseMovement') {
    return {
      backgroundColor: 'rgba(255, 152, 0, 0.6)',
      borderColor: 'rgba(255, 152, 0, 1)'
    };
  } else if (xAxisMetric.value === 'keyboardInput' || yAxisMetric.value === 'keyboardInput') {
    return {
...
function initChart() {
  ...
  const bubbleData = props.participantData.map(participant => {
    ...
    const sizeMultiplier = (xAxisMetric.value === 'mouseMovement' || yAxisMetric.value === 'mouseMovement') ? 1.2 : 1;
...
watch(() => props.studyId, (newStudyId) => {
  if (newStudyId) {
